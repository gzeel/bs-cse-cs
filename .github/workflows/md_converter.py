    ...
    for match in re.finditer(r'<img[^>]*src="([^"]*)"[^>]*>', content):
        ...
        if img_path.startswith('images/'):
            ...
            if os.path.exists(full_path):
                ...
                alt_path = os.path.join(md_dir, base_name)
                if os.path.exists(alt_path):
                    print(f"  Found HTML image in current directory instead: {alt_path}")
                    images.append((alt_path, f"images/{base_name}"))
        else:
            # Check if image is in the same directory
            direct_path = os.path.join(md_dir, img_path)
...
def convert_markdown_to_html(md_file):
    ...
    try:
        ...
        with open(temp_js_file, 'w', encoding='utf-8') as f:
            ...
            // Read markdown content
            const mdContent = fs.readFileSync("{temp_md_file}", "utf8");
            
            // Convert to HTML
            converter.convert(mdContent, "{title}")
                .then(html => {
        ...
        for img in soup.find_all('img'):
            ...
            if not src.startswith(('http://', 'https://', 'data:', 'images/')):
                ...
                print(f"  Updated image path in HTML: {src} -> images/{os.path.basename(src)}")
        ...
        if images:
            ...
            for src_path, target_rel_path in images:
                if os.path.exists(src_path):
                    target_path = os.path.join(os.path.dirname(html_path), target_rel_path)
...
def copy_to_docs_branch():
    ...
    result = subprocess.run(['git', 'ls-remote', '--heads', 'origin', 'docs'], 
    ...
    if docs_exists:
        ...
        subprocess.run(['git', 'commit', '-m', 'Initial docs branch commit'], check=True)
